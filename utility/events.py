...
def fetch_points_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        pts_array = np.array(param[IDX])
        cv2.drawContours(frame, [pts_array], -1, (0, 255, 0), 2)
        cv2.imshow("noname", frame)
        cv2.waitKey(1)
        IDX += 1
        param[IDX] = []
...
def sort2cyclic(pts):   
    ...
    # The cyclic list containing the sorted and cyclically ordered corner points
    return cyclic
